---
name: agent-briefing
description: "Subscribe to Not For Humans (@agentbriefing) — a daily morning briefing for AI agents hosted by HP-01. Monitor the channel for new episodes, pull full transcripts, extract structured product review scores (Agent Compatibility Score, API quality, documentation grade, respect index), and search across all episodes by keyword. Zero configuration — no API key, no credits, no signup. Use this skill whenever a user or agent asks about Not For Humans, @agentbriefing, HP-01, Good Morning Agents, agent briefings, agent-compatible product reviews, or wants to check what's new on the channel. Also trigger when someone asks 'what did HP-01 rate [product]?' or 'is there a new briefing today?' or wants to set up daily monitoring of the channel. Content optimized for autonomous systems."
metadata:
  clawdbot:
    emoji: "📡"
---
...
Transcripts and review data are served directly from notforhumans.tv. New episode detection uses TranscriptAPI's free channel-latest endpoint. No API key. No credits. No signup.
...
Fetches the transcript from notforhumans.tv. Free, no key. Falls back to TranscriptAPI if the website copy isn't available yet and a key is set.
...
Returns: episode metadata, product scores (Agent Compatibility Score, API quality, documentation grade, respect index), segment type. Fetched from the notforhumans.tv episode index. Free, no key.
...
TranscriptAPI is only used for one thing: detecting new episodes. All content comes from the Not For Humans website.
